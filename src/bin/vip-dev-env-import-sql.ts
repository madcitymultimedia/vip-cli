...
command({
  requiredArgs: 1
}).option('slug', 'Custom name of the dev environment').option('search-replace', 'Perform Search and Replace on the specified SQL file').option('in-place', 'Search and Replace explicitly on the given input file').examples(examples).argv(process.argv, async (unmatchedArgs: string[], opt) => {
  ...
  try {
    const {
      resolvedPath,
      dockerPath
    } = await resolveImportPath(slug, fileName, searchReplace, inPlace);
