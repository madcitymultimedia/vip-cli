...
import { currentUserCanImportForApp, isSupportedApp, SQL_IMPORT_FILE_SIZE_LIMIT, SQL_IMPORT_FILE_SIZE_LIMIT_LAUNCHED } from "lib/site-import/db-file-import";
...
export async function gates(app: AppForImport, env: EnvForImport, fileName: string) {
  ...
  const maxFileSize = env?.launched ? SQL_IMPORT_FILE_SIZE_LIMIT_LAUNCHED : SQL_IMPORT_FILE_SIZE_LIMIT;
...
const promptToContinue = async ({
  ...
  const promptResponse = await prompt({
    ...
    message: `You are about to import the above tables into a ${launched ? 'launched' : 'un-launched'} ${formattedEnvironment} site ${chalk.yellow(domain)}.\nType '${chalk.yellow(promptToMatch)}' (without the quotes) to continue:\n`
  });
  ...
  if (!tableNames.length) {
    ...
    if (!isMultiSite) {
      ...
      if (siteArray === 'undefined' || !siteArray) {
        console.log(chalk.yellowBright('Unable to determine the subsites affected by this import, please proceed only if you are confident on the contents in the import file.'));
        return;
      ...
      if (launched) {
        console.log(chalk.yellowBright('You are updating tables in a launched multi site installation. Sites in the same network may have their performance impacted by this operation.'));
      }
...
`;
    progressTracker.suffix = `\n${getGlyphForStatus(status, progressTracker.runningSprite)} ${status === 'running' ? 'Loading remaining steps' : ''}`; // TODO: maybe use progress tracker status
  };
