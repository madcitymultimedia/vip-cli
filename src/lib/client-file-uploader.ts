...
export interface GetSignedUploadRequestDataArgs {
  action: "AbortMultipartUpload" | "CreateMultipartUpload" | "CompleteMultipartUpload" | "ListParts" | "PutObject" | "UploadPart";
  etagResults?: Array<Record<string, any>>;
  appId: number;
  envId: number;
  basename: string;
  partNumber?: number;
...
export const getWorkingTempDir = async () => new Promise((resolve, reject) => {
...
export const getFileMD5Hash = async (fileName: string) => new Promise((resolve, reject) => fs.createReadStream(fileName).pipe(createHash('md5').setEncoding('hex')).on('finish', function () {
  resolve(this.read());
}).on('error', error => reject(`could not generate file hash: ${error}`)));
export const gzipFile = async (uncompressedFileName: string, compressedFileName: string) => new Promise((resolve, reject) => fs.createReadStream(uncompressedFileName).pipe(createGzip()).pipe(fs.createWriteStream(compressedFileName)).on('finish', resolve).on('error', error => reject(`could not compress file: ${error}`)));
...
export async function uploadImportSqlFileToS3({
  ...
  debug(`File ${chalk.cyan(fileMeta.basename)} is ~ ${Math.floor(fileMeta.fileSize / MB_IN_BYTES)} MB\n`);
  ...
  if (!fileMeta.isCompressed && fileMeta.fileSize >= COMPRESS_THRESHOLD) {
    ...
    const calculation = `${(fewerBytes / MB_IN_BYTES).toFixed(2)}MB (${Math.floor(100 * fewerBytes / uncompressedFileSize)}%)`;
...
export async function uploadUsingPutObject({
  ...
  const response = await fetch(presignedRequest.url, { ...fetchOptions,
...
}: GetSignedUploadRequestDataArgs): Promise<Record<string, any>> {
  const {
    apiFetch
  } = await API();
...
export async function uploadParts({
  ...
  const updateProgress = () => {
    ...
    debug(partPercentages.map((partPercentage, index) => {
      ...
      return `Part # ${index}: ${partPercentage}% of ${(partSize / MB_IN_BYTES).toFixed(2)}MB`;
    }).join('\n') + `\n\nOverall Progress: ${percentage}% of ${(fileMeta.fileSize / MB_IN_BYTES).toFixed(2)}MB`);
  };
...
export async function uploadPart({
  ...
  const doUpload = async () => {
    ...
    fetchOptions.body = fs.createReadStream(fileName, {
      start,
      end
...
export async function completeMultipartUpload({
  ...
  const completeMultipartUploadResponse = await fetch(completeMultipartUploadRequestData.url, completeMultipartUploadRequestData.options);
